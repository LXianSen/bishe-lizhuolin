...
public class CartDao extends BaseDAO<cartitem> {
	...
	public double getsumPrice(cartitem cartitem) throws SQLException, Exception {
		...
		double price = 0;
		String sql = "select sum(bprice) from cartitem where userid ='"+cartitem.getUserid()+"'";
	...
	public double getsumdiscountPrice(cartitem cartitem) throws SQLException, Exception {
		Connection connection=Druid().getConnection();
		double price = 0;
		String sql = "select sum(bprice*bdiscount) from cartitem where userid = '"+cartitem.getUserid()+"'";
		PreparedStatement ps=connection.prepareStatement(sql);
		ResultSet rs = ps.executeQuery();
		try {
			if (rs.next()) {
