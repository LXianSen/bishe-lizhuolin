...
public class CartDao extends BaseDAO<cartitem> {
	...
	public void updateNum(cartitem cartitem) throws Exception {
		...
		String sql = "update cartitem set count = "+cartitem.getCount()+" where userid = "+cartitem.getuserId()+" and isbn = "+cartitem.getIsbn()+"";
	...
	public void clear(cartitem cartitem) throws SQLException, Exception {
		Connection connection=Druid().getConnection();
		String sql = "delete from cartitem where userid = "+cartitem.getuserId()+"";
		PreparedStatement ps=connection.prepareStatement(sql);
		ResultSet rs=ps.executeQuery();

	}
	//查询购物车所有元素
	public List<cartitem> getAllItems(cartitem cartitem) throws SQLException, Exception {
		Connection connection=Druid().getConnection();
		List<cartitem> list = new ArrayList<cartitem>();
		String sql = "select * from cartitem where userId="+cartitem.getuserId()+"";
		PreparedStatement ps=connection.prepareStatement(sql);
		ResultSet rs = ps.executeQuery();
		try {
			while (rs.next()) {
				cartitem cartitem2 =new cartitem(rs.getString(1),rs.getString(2),rs.getDouble(3),rs.getInt(4));
				list.add(cartitem2);
			}
	...
	//查询用户某本书
	public cartitem findById(cartitem cartitem) throws SQLException, Exception {
		cartitem cartitem2=new cartitem();
		Connection connection=Druid().getConnection();
		String sql = "select * from cartitem where userid = "+cartitem.getuserId()+" and isbn = "+cartitem.getIsbn()+"";
		PreparedStatement ps=connection.prepareStatement(sql);
		ResultSet rs=ps.executeQuery();
		try {
			if (rs.next()) {
				cartitem2 = new cartitem(rs.getString(1), rs.getString(2), rs.getDouble(3),rs.getInt(4));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
